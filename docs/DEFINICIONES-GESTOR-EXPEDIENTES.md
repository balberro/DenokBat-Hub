...
- **Estados V1**: `borrador` → `completa` → `aceptada`.
  - `borrador`: en redacción.
  - `completa`: lista para firma (el proceso de firma es externo y puede tardar).
  - `aceptada`: se ha subido el PDF firmado (documento externo). El estado legado `firmada` se normaliza a `aceptada`.
...
- **Aceptar acta (firma)**: al subir el PDF firmado, el acta pasa a `aceptada` y el PDF se archiva en la tabla `db_actas_pdf` (relación 1:1).
...
- **PDF firmado en tabla aparte**: desde la refactorización `aceptada`, el PDF firmado vive en `db_actas_pdf` (1:1 con el acta), no en columnas de `db_actas`. Migración idempotente: `lib/db/fix-db-actas-aceptada.sql`.
...
| 2026-05-25 | **Archivo PDF del acta firmada** (V1). El acta se firma subiendo el **PDF firmado** desde el modal `Firmar acta y archivar PDF`. El archivo se guarda en `artifacts/api-server/uploads/actas/<YYYY>/<MM>/` con nombre `YYYY-MM-<slug-del-titulo>.pdf` para que la búsqueda alfabética coincida con el orden cronológico. Si ya hay otra acta firmada con la misma clave `YYYY-MM`, se muestra el aviso de duplicados (lista con número, título, fecha y enlace) y el nuevo PDF recibe sufijo numérico (`-2`, `-3`, ...). Endpoints `GET /admin/actas/:id/duplicados-mes` (preconsulta de duplicados) y `POST /admin/actas/:id/firmar` (body `{ pdf_data_url }`, valida `data:application/pdf;base64,...`, tope de 25 MB, devuelve `suffix` y `duplicados_mes`). En la ficha del acta firmada, en el histórico de socios y en la vista de impresión aparece el botón **Descargar PDF**. |
| 2026-05-26 | **Refactorización del PDF firmado + estado `aceptada`**. El estado pasa de `firmada` a `aceptada` (el `firmada` legado se normaliza). El PDF firmado deja de vivir en columnas de `db_actas` y se mueve a la tabla `db_actas_pdf` (relación 1:1, con `pdf_url`, `pdf_filename`, `pdf_anyo_mes`, `pdf_size`, `subido_en`, `subido_por`), con índice parcial por `pdf_anyo_mes`. La API sigue exponiendo los campos `pdf_*` al cliente vía `LEFT JOIN db_actas_pdf`. Migración idempotente: `lib/db/fix-db-actas-aceptada.sql` (crea la tabla, migra desde las columnas antiguas `db_actas.pdf_*` si existen y actualiza el CHECK de estado). `lib/db/fix-db-actas-pdf.sql` queda **deprecado**. |
